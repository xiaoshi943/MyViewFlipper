...
public class SwitchButton extends CompoundButton {
	...
	//
	// colors = appearance
	// .getColorStateList(R.styleable.TextAppearance_swi_textColor);
	// if (colors != null) {
	// mTextColors = colors;
	// } else {
	...
	// R.styleable.TextAppearance_swi_textSize, 60);
	//
	// if (ts != 0) {
	...
	// R.styleable.TextAppearance_swi_textStyle, -1);
	//
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// mOffLayout.getWidth());
		// 该控件的宽度：字符串长度*2+字符串的padding*4+点9左右边距
		/*
	...
	public void populateAccessibilityEvent(AccessibilityEvent event) {
		/*
		 * if (isChecked()) { CharSequence text = mOnLayout.getText(); if
		 * (TextUtils.isEmpty(text)) { text =
		 * mContext.getString(R.string.switch_on); } event.getText().add(text);
		 * } else { CharSequence text = mOffLayout.getText(); if
		 * (TextUtils.isEmpty(text)) { text =
		 * mContext.getString(R.string.switch_off); } event.getText().add(text);
		 * }
		 */
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			case TOUCH_MODE_DOWN: {
				...
				final float y = ev.getY();
				// 去掉mTouchSlop
				if (Math.abs(x - mTouchX) > 0 || Math.abs(y - mTouchY) > 0) {
	...
	protected void onDraw(Canvas canvas) {
		...
		// switchBottom);
		mTrackDrawable.setBounds(0,
				(mThumbDrawable.getIntrinsicHeight() - mTrackDrawable.getIntrinsicHeight()) / 2 + getPaddingTop(), 
				mTrackDrawable.getIntrinsicWidth() + getPaddingLeft(),
				(mThumbDrawable.getIntrinsicHeight() + mTrackDrawable.getIntrinsicHeight())/ 2 + getPaddingTop());
	...
	private int getThumbScrollRange() {
		...
		return mSwitchWidth - mThumbDrawable.getIntrinsicWidth();
	}
